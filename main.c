 ///
 /// @file    main.c
 /// @author  lemon(dortmund_reus@163.com)
 /// @date    2020-08-29 18:43:48
 ///
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main()
{
    ...
    { 
		...
		处理 phandle.buff
		*/
	//	printf("receive %d bytes messages.\n", phandle.rxbuffsize);
		for(int i = 0; i < phandle.rxbuffsize; ++i)
		{
			if(CopeSerialData(phandle.buff[i]))
			{
	//			printf("Acc:%f %f %f\n", (float)stcAcc.a[0]/32768*16, (float)stcAcc.a[1]/32768*16, (float)stcAcc.a[2]/32768*16);
				printf("Time:20%hhu-%hhu-%hhu %hhu:%hhu:%f\n", stcTime.ucYear, stcTime.ucMonth, stcTime.ucDay, stcTime.ucHour, stcTime.ucMinute, (float)stcTime.ucSecond+(float)stcTime.usMiliSecond/1000);
				phandle.rxbuffsize = 0;
			}
//			printf("%hhu", phandle.buff[i]);
		}
	}
	return 0;
}
